...
class BCA {
  ...
  async request({
    ...
    const timestamp = format(new Date(), bca.timestampFormat);
    ...
    return this.apiCall({
      ...
      body,
      port: bca.api_port,
      url: `${bca.api_host}${relativeUrl}`,
      headers: {
        Authorization: `Bearer ${token.access_token}`,
        'Content-Type': 'application/json',
        'Origin': bca.origin,
        'X-BCA-Key': bca.api_key,
  ...
  async getSignature({
    ...
    const hmac = crypto.createHmac('sha256', bca.api_secret);
